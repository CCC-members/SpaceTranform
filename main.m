...
sources_iv              = sources_iv/max(sources_iv(:));
fig3 = plotBrainSurface(sources_iv, Vertices10, Faces10);
%% null model and plot zmap
[z_map, nulls] = generateSpatialNulls(sources_iv, 'n_nulls', 1000);
fig4 = plotBrainSurface(abs(z_map), Vertices10, Faces10);

%% save gii
% Example 1: Save both hemispheres (默认等分)
saveAsGIfTI(sources_iv, './results', 'fsaverage10k');

% Example 2: 保存单半球数据
saveAsGIfTI(left_hemisphere_data, './results', 'left_data', 'hemi', 'lh');
saveAsGIfTI(right_hemisphere_data, './results', 'right_data', 'hemi', 'rh');

% Example 3: 使用自定义分割点
saveAsGIfTI(data_vector, './results', 'custom_split', ...
    'split_point', 15000, ...
    'verbose', true);